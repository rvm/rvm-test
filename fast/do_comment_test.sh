...
: -----------------------------------------------------------------
mkdir -p $d/1.8.7
echo "echo 1.8.7" > $d/1.8.7/.ruby-version
...
ver=1.9.3
mkdir -p $d/1.9.3
echo "echo 1.9.3" > $d/1.9.3/.ruby-version

: absolute directory
rvm in $d/1.9.3 do rvm info ruby # status=0; match=/version: *"1.9.3/
rvm in $d/1.9.3 do ruby --version # status=0; match=/^ruby 1.9.3 /

: relative directory
cd $d
rvm in 1.9.3 do rvm info ruby # status=0; match=/version: *"1.9.3/
rvm in 1.9.3 do ruby --version # status=0; match=/^ruby 1.9.3 /

: current directory
cd $d/1.9.3
rvm . do rvm info ruby # status=0; match=/version: *"1.9.3/
rvm . do ruby --version # status=0; match=/^ruby 1.9.3 /
